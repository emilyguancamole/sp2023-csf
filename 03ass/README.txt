...
However, the total cache capacity was kept at 2^10=1024, and each block consistently contained 16 bytes of memory.
...
We then compared results between write policies, keeping other parameters of each cache constant. 
    ...
    Compared to write-through, write-back had significantly fewer total cycles. For instance, for set-assoc & lru, we saw a difference of 13-15 million cycles.

    Compared to write-allocate, no-write-allocate had more load misses (ex. direct mapping & lru, 9000 vs. 6000) 
    and store misses (35000 vs. 10000).
...
Finally, we did further comparisons in results for lru vs. fifo eviction.
...
From our experiments and analysis, we conclude that the cache configuration with the best overall effectiveness is 
a set-associative cache that uses write-back + write-allocate write policies and lru eviction.