...
void Connection::connect(const std::string &hostname, int port) {
  ...
  m_fd = open_clientfd(hostname_c, port_c); //? accepts hostname (server address) as a string and the desired port as a string
     //* ^ returns a file descriptor (the 'socket') - client socket
    // ????? what is m_fd and why do we use it
  if (m_fd < 0) {
...
bool Connection::send(const Message &msg) {
  ...
  if (msg_size == msg_size_sent) {
    m_last_result = SUCCESS;
  } else { //? should we make sure msg_size_sent is > 0?
